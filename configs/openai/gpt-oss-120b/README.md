...
```bash
helm upgrade -i gpt-oss-120b \
  --values configs/openai/gpt-oss-120b/values.yaml \
  ./configs/chart
```
...
```bash
kubectl port-forward svc/gpt-oss-120b 8080
```
...
```bash
kubectl port-forward svc/gpt-oss-120b 8000
```
