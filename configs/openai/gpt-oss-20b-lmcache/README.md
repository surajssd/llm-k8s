...
```bash
helm upgrade -i gpt-oss-20b-lmcache \
  --values configs/openai/gpt-oss-20b-lmcache/values.yaml \
  ./configs/chart
```
...
```bash
kubectl port-forward svc/gpt-oss-20b-lmcache 8080
```
...
```bash
kubectl port-forward svc/gpt-oss-20b-lmcache 8000
```
